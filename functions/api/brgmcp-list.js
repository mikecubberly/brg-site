// /api/brgmcp-list
// Returns the BRGmcp submissions index (last 500), most recent first.
// Auth: header X-Auth-Token or ?token= query param, must equal env.MCP_DIAG_TOKEN.

export async function onRequestGet({ request, env }) {
  const cors = {
    'Access-Control-Allow-Origin': '*',
    'Content-Type': 'application/json',
    'Cache-Control': 'no-store'
  };

  if (!env.MCP_DIAG_TOKEN) {
    return new Response(JSON.stringify({ error: 'Auth not configured' }), { status: 503, headers: cors });
  }
  if (!env.BRGmcp_KV) {
    return new Response(JSON.stringify({ error: 'Storage not configured' }), { status: 500, headers: cors });
  }

  const url = new URL(request.url);
  const provided = request.headers.get('X-Auth-Token') || url.searchParams.get('token') || '';
  if (provided !== env.MCP_DIAG_TOKEN) {
    return new Response(JSON.stringify({ error: 'Unauthorized' }), { status: 401, headers: cors });
  }

  try {
    const raw = await env.BRGmcp_KV.get('brgmcp:index');
    const submissions = raw ? JSON.parse(raw) : [];
    return new Response(JSON.stringify({ ok: true, count: submissions.length, submissions }), { status: 200, headers: cors });
  } catch (e) {
    return new Response(JSON.stringify({ error: 'Failed to read index' }), { status: 500, headers: cors });
  }
}
